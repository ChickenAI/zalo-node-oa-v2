...
/**
 * Zalo OA API v2.0 Implementation
 *
 * This implementation uses the v2.0 API base URL and message endpoints
 * following the MessageV2 API format.
 */
			...
			try {
				if (resource === 'zaloOA') {
					...
					// Lấy thông tin người theo dõi
						...
						try {
							// Sử dụng API v2.0 để lấy thông tin người theo dõi
							...
							// Log lỗi để debug
							console.log('Lỗi khi gọi API getprofile v2.0:', error.message);
							...
							returnData.push({
								json: {
									...
									suggestion: 'Hãy đảm bảo rằng bạn đã cấp quyền truy cập cho ứng dụng và có access_token hợp lệ.',
								},
								pairedItem: {
									item: i,
								},
					...
					// Lấy danh sách tag
						...
						try {
							...
							try {
								// Thử gọi lại API v2.0 với cấu hình khác
								const response = await axios.get(`${baseUrl}/tag/gettagsofoa`, {
									headers: {
										access_token: accessToken,
									},
					...
					// Lấy thông tin Official Account
						...
						try {
							...
							// Log lỗi để debug
							console.log('Lỗi khi gọi API getoa v2.0:', error.message);
							console.log('Đang thử lại...');

							// Thử gọi lại API với cấu hình khác
							const response = await axios.get(
								`${baseUrl}/getoa`,
								{
									headers: {
					...
					// Lấy danh sách người theo dõi
						...
						try {
							// Sử dụng API v2.0 để lấy danh sách người theo dõi
